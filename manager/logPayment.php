...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    try {
        foreach ($items as $item) {
            ...
            $stmt = $dbh->prepare("SELECT itemQuantity, itemPrice, itemCategory FROM inventoryItem WHERE itemCode = ? FOR UPDATE");
        ...
        $stmt = $dbh->prepare("INSERT INTO logPayment (userID, paymentAmount, paymentMethod, transactionType, discount) VALUES (?, ?, ?, ?, ?)");
        $stmt->execute([$userID, $discountedTotal, $paymentMethod, $transactionType, $discount]);
        ...
        foreach ($items as $item) {
            $itemCode = intval($item['itemCode']);
            $qty = intval($item['qty']);
            $updateStmt = $dbh->prepare("UPDATE inventoryItem SET itemQuantity = itemQuantity - ? WHERE itemCode = ?");
            $updateStmt->execute([$qty, $itemCode]);
        }
...
$stmt = $dbh->query("SELECT itemCode, itemName, itemPrice, itemQuantity, itemCategory FROM inventoryItem 
